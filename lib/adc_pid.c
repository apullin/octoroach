...
//TODO (pullin, fgb) : this is not expected to work without an overhaul for 33E

static void adcSetupPeripheral(void) {
    unsigned int AD1CON1value, AD1CON2value, AD1CON3value, AD1CON4value,
            AD1CSSHvalue, AD1CSSLvalue, AD1CHS0value, AD1CHS123value;
    ...
    AD1CON1value = ADC_MODULE_ON & //ADC module is enabled
            ...
            ADC_AD12B_10BIT & // ADC in 10 bit mode
            ADC_FORMAT_INTG & // ADC in integer format (CLARIFY)
            ADC_SSRC_PRI_SEVT & // TODO (pullin, fgb) : major change for ADC core for dspic33E
            ADC_SIMULTANEOUS & //Simultaneously sample CH0 and CH1
            ADC_AUTO_SAMPLING_ON & //ADC does not need to be triggered manually
            ADC_SAMP_ON; //sample / hold amplifiers are sampling  (maybe incorrect)
    ...
    AD1CON3value = ADC_SAMPLE_TIME_1 & //Sample Time = 1*Tad
            ADC_CONV_CLK_SYSTEM & //Use System clock, not internal RC osc
            ADC_CONV_CLK_3Tcy; //Tad = 3 * Tcy
    ...
    OpenADC1(AD1CON1value, AD1CON2value, AD1CON3value, AD1CON4value,
            0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,//Ports ABCDEFGHIJK all digital
            AD1CSSHvalue, AD1CSSLvalue);
    
    //Enable specific analog pin function
    ANSELBbits.ANSB0 = 1;
    ANSELBbits.ANSB1 = 1;
    ANSELBbits.ANSB3 = 1;
    ANSELBbits.ANSB11 = 1;
...
__eds__ unsigned int BufferA[4][SAMP_BUFF_SIZE] __attribute__((eds,space(dma)));
__eds__ unsigned int BufferB[4][SAMP_BUFF_SIZE] __attribute__((eds,space(dma)));
